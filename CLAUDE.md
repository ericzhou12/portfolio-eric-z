...
honors: honors and skills just text.

code is organized by feature:
src/features/{home,experience,honors,music}/ — components + their own lib files, colocated
src/components/ — shared chrome (header, footer, social rail, theme, command palette) and primitives (reveal, section)
src/lib/ — content.ts (all site copy, single source of truth), utils.ts, use-modifier-key.ts
